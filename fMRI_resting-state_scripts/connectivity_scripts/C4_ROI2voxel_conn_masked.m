function C4_ROI2voxel_conn_masked(SJs,runs,src_dir,prefix_func,atlas,seed_ROI,mask_file,sessNum)
...
for s = 1:length(SJs)
    ...
    mkdir(outputdir);
    if sessNum > 0 && esist([src_dir filesep SJs{s} filesep 'ses-1' filesep 'func'])
        for ses = 1:sessNum
            for r = 1:size(runs,2)
                run=runs{s,r};
                ...
                for f=1:size(f1,1)
                    ...
                    % save Matlab-data
                    eval(['save ' outputdir filesep 'R2Vconn_' tempfile(1:end-4) '.mat corrval pval tval dim x y z subject run atlas seed_ROI tempfile roi_name'])
