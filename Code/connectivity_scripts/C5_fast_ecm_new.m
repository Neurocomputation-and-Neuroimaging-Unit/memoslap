function C5_fast_ecm_new(SJ, runs, run_dir, prefix_func, mask, ztransform, smooth, kernel_size, label, out_dir)
% Compute Eigen-Centrality-Maps for a single subject/session

% initialise SPM jobman to prevent interactive GUIs
spm('defaults', 'FMRI');
spm_jobman('initcfg');

% check GM mask dimensions match functional data space
...
if any(V_func(1).dim ~= V_mask.dim)
    error(['GM mask dimensions %s do not match functional data dimensions %s.\n'], ...
...
subject = SJ;
ECM_dir = fullfile(fileparts(out_dir), ['ECM_results' label]);
mkdir(ECM_dir)
...
for r = 1:length(runs)
    ...
    for f = 1:size(f1,1)
        ...
        file2transform = spm_select('List', run_dir, ['^fastECM_.*\.nii$']);
        ...
        if smooth
            ...
            matlabbatch{1}.spm.spatial.smooth.data   = {[run_dir filesep strtrim(file2transform)]};
        ...
        % rename ECM output files to include label
        if ~isempty(label)
            ecm_files = dir(fullfile(run_dir, '*ECM*.nii'));
            for i = 1:length(ecm_files)
                [ep, en, ee] = fileparts(ecm_files(i).name);
                movefile(fullfile(run_dir, ecm_files(i).name), ...
                         fullfile(run_dir, [en label ee]));
            end
        end

        % Move all ECM files to ECM_dir
        ecm_files_final = dir(fullfile(run_dir, '*ECM*.nii'));
        for i = 1:length(ecm_files_final)
            movefile(fullfile(run_dir, ecm_files_final(i).name), ECM_dir);
        end
    end
