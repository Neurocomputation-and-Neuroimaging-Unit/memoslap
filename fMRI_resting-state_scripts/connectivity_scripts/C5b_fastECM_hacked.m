function C5b_fastECM_hacked( inputfile, rankmap, normmap, degmap, maxiter, maskfile, atlasfile, scans )

% taken from fastECM Toolbox
% hacked by Michal to allow for sliding window analysis
...
M=load_untouch_nii(inputfile,scans);        % read the information from the NifTI file
%######################################
% edited by till, 2015-12-18
% datatype single
m=single(M.img);                      % get the 4d voxel data
M.hdr.dime.datatype=16;
M.hdr.dime.bitpix=32;
%######################################
M.img=[];                             % empty img after reading
M=rmfield(M,'img');                   % and remove from M
M.hdr.dime.dim(1)=3;                  % 3d for output
msz=size(m);
...
%     msk=int16(Msk.img);                      % edited by till, 2015-12-18
...
qnts=quantile(vcurr(vcurr>0),[.01 .99]);    % quantiles to set contrast

% write output nifti file based on mout

[fd,fn,fx2]=fileparts(inputfile);
if ~strcmp('.nii',fx2)  %if last extension is not .nii
    [fd1,fn,fx]=fileparts(fn);            % base file name and 1st extension (2nd may be .gz)
    fx=[fx fx2];                          % in which case, concatenate by adding fx2
else
    fx=fx2;
