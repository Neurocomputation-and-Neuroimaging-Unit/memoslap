function D2_FIR(data_dir, onsets, TR, WMdelay, currPrefix, runs, fir_out, condnames, hm, hpf)
...
for s = 1:size(runs,2)
    ...
    jobs{1}.stats{1}.fmri_spec.sess(s).hpf     = hpf; %300; %128;
    ...
    if hm                
        ...
        f1=spm_select('List', nifti_dir, ['^rp_' runs{runID}(1:k-1) '.txt']);            
        if ~isempty(f1)
