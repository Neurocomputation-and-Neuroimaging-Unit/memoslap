function C2_glm_1stLevel(subj_dir, outputfolder_1st, prefix_func, tr, fmri_t, fmri_t0, hpf, runs, condnames, onsets, duration, hm, cc)
...
for r = 1:size(runs,2)
    ...
    if exist(run_dir)==7
        ...
        if hm
            ...
            n=1;
            k = strfind(runs{r}, 'd.nii')
            f1=spm_select('List', run_dir, ['^rp_' prefix_func(n:end) runs{r}(1:k) '.txt']);
            while isempty(f1)
                n=n+1;
                f1=spm_select('List', run_dir, ['^rp_' prefix_func(n:end) runs{r}(1:k) '.txt']);
            end
            ...
            if ~isempty(f1)
                ...
                display('################################################################################')
                display(['############### ' run_dir ', ' runs{r} ': Headmotion Parameters do not exsist #############'])
                display('################################################################################')
            end
        end
        ...
        if cc
            k = strfind(runs{r}, 'd.nii')
            f2=spm_select('List',run_dir,['^' prefix_func runs{r}(1:k) '_CompCorPCs.txt']);
            n=1;
            while isempty(f2)
                f2=spm_select('List',run_dir,['^' prefix_func(n:end) runs{r}(1:k) '_CompCorPCs.txt']);
                n=n+1;
            end
            if ~isempty(f2)
        ...
        for c = 1:length(condnames)
            ...
            matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(c).duration = duration(c);            
...
matlabbatch{1}.spm.stats.fmri_spec.mask = {'/scratch/grum90/memoslap/masks/mask_skull.nii'}; % {''}; % explicit mask
