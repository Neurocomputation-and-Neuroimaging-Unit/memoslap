...
%scanning 
function B13_cut_data(src_dir, TR, nifti_files, segment_length, segment_start)
...
for file = 1:length(nifti_files)
    tempdir= [nifti_files(file).folder];
    cd(tempdir)
    f = spm_select('List',tempdir, ['^' nifti_files(file).name]);
    ...
    if (current_TR + segment_size-1) <= nr_scans
        display(['file ' nifti_files(file).name ', segment ' num2str(current_TR) '-' num2str(current_TR + segment_size-1)])
